...
create table Xuat(
       SoHDX nvarchar(30),
       MaSP nvarchar(20),
       constraint PKX primary key(SoHDX,MaSP),
       SoLuongX int,
...
('SP01', 'H01', N'Galaxy S', 100, 'Đen', 20000000, 'Chiếc', 'Điện thoại Samsung'),
('SP02', 'H01', N'Tivi QLED', 50, 'Đen', 15000000, 'Chiếc', 'Tivi 55 inch'),
('SP03', 'H02', N'iPhone 15', 30, 'Đỏ', 25000000, 'Chiếc', 'Điện thoại mới'),
('SP04', 'H03', N'Tai nghe WH-1000XM4', 100, 'Đen', 7000000, 'Chiếc', 'Tai nghe chống ồn');
...
('HDN01', '2019-05-10', 'NV01'), -- để test top 10 số lượng 2019
('HDN02', '2019-07-20', 'NV01'),
('HDN03', '2020-02-01', 'NV02');

-- Nhập chi tiết
insert into Nhap(SoHDN, MaSP, SoLuongN, DonGiaN) values
('HDN01', 'SP01', 500, 18000000), -- Galaxy S nhiều nhất
('HDN01', 'SP02', 400, 14000000),
('HDN02', 'SP03', 300, 24000000),
('HDN02', 'SP04', 200, 6500000),
('HDN03', 'SP02', 50, 14500000); -- cho truy vấn MaSP=SP02 nhân viên NV02

-- Phiếu xuất
insert into PXuat(SoHDX, NgayXuat, MaNV) values
('HDX01', '2020-02-03', 'NV02'), -- SP02 xuất ngày 03/02/2020
('HDX02', '2023-06-15', 'NV01'), -- Galaxy S xuất năm 2023
('HDX03', '2023-02-03', 'NV03'); -- SP04 xuất ngày 3/2/2023
...
ORDER BY n.SoLuongN DESC;
 --Đưa ra MaSP,TenSP của các sản phẩm do công ty ‘Samsung’ sản xuất do nhân viêncó mã ‘NV01’ nhập.
...
WHERE hs.TenHang = N'Samsung'
  AND pn.MaNV = 'NV01';
 ---Đưa ra SoHDN,MaSP,SoLuongN,ngayN của mặt hàng có MaSP là ‘SP02’, được nhânviên ‘NV02’ xuất.
...
WHERE x.MaSP = 'SP02'
 ...
 --Đưa ra Các sản phẩm do hãng SamSung sản xuất
 SELECT sp.*
FROM SanPham sp
JOIN HangSX hs ON sp.MaHangSX = hs.MaHangSX
WHERE hs.TenHang = N'Samsung';
 --Đưa ra Nhân viên đã xuất sản phẩm “Galaxy S” trong năm 2023
 SELECT DISTINCT nv.MaNV, nv.TenNV
FROM NhanVien nv
JOIN PXuat px ON nv.MaNV = px.MaNV
JOIN Xuat x ON px.SoHDX = x.SoHDX
...
WHERE sp.TenSP = N'Galaxy S' AND YEAR(px.NgayXuat) = 2023;
 --Đưa ra thông tin sản phẩm được xuất ngày “3/2/2023”
 SELECT sp.*
FROM SanPham sp
